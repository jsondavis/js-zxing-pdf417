...
if (typeof $Inherit == "undefined") {
    var $Inherit = function(ce, ce2) {
        if (typeof Object.getOwnPropertyNames == "undefined") {
            ...
            for (var p in ce2) if (typeof ce[p] == "undefined") ce[p] = ce2[p];
            ce.$baseCtor = ce2;
        } else {
...
var ZXing =
    typeof exports != "undefined" && typeof exports.ZXing != "undefined"
        ? exports.ZXing
        : {};

if (typeof exports != "undefined") exports.ZXing = ZXing;

if (typeof ZXing.Common == "undefined") ZXing.Common = {};

if (typeof ZXing.Common.Detector == "undefined") ZXing.Common.Detector = {};

if (typeof ZXing.PDF417 == "undefined") ZXing.PDF417 = {};

if (typeof ZXing.PDF417.Internal == "undefined") ZXing.PDF417.Internal = {};

if (typeof ZXing.PDF417.Internal.EC == "undefined")
    ZXing.PDF417.Internal.EC = {};

ZXing.SupportClass = function() {};
ZXing.SupportClass.GetCharsFromString = function(
    sourceString,
    sourceStart,
    sourceEnd,
    destinationArray,
    ...
    while (sourceCounter < sourceEnd) {
        destinationArray[destinationCounter] = sourceString.charAt(
            sourceCounter
        );
        sourceCounter++;
        destinationCounter++;
...
ZXing.SupportClass.SetCapacity = function(vector, newCapacity) {
    while (newCapacity > vector.length) vector.push(new T());
...
ZXing.SupportClass.toStringArray = function(strings) {
    ...
    var result = [];
    for (var val in obj) {
        if (!(obj[val] instanceof Function)) result.push(obj[val]);
    }
    return result;
};
...
ZXing.ResultPoint.distance = function(pattern1, pattern2) {
    return ZXing.Common.Detector.MathUtils.distance(
        pattern1.x,
        pattern1.y,
        pattern2.x,
        pattern2.y
...
ZXing.BitmapLuminanceSource.prototype.CreateLuminanceSource = function(
    newLuminances,
    width,
    height
) {
...
ZXing.Common.DecoderResult = function(
    rawBytes,
    text,
    byteSegments,
    ecLevel,
    ...
    this.StructuredAppendParity =
        typeof saParity == "undefined" ? -1 : saParity;
    this.StructuredAppendSequenceNumber =
        typeof saSequence == "undefined" ? -1 : saSequence;
...
ZXing.Common.BitArray = function(bits, size) {
    ...
    this.bits =
        arguments.length == 2
            ? bits
            : this.size
            ? ZXing.Common.BitArray.makeArray(this.size)
            : [];
};
ZXing.Common.BitArray._lookup = new Int32Array([
    32,
    0,
    1,
    26,
...
ZXing.Common.BitArray.prototype.getNextSet = function(from) {
    ...
    var result =
        (bitsOffset << 5) +
        ZXing.Common.BitArray.numberOfTrailingZeros(currentBits);
...
ZXing.Common.BitArray.prototype.toBytes = function(
    bitOffset,
    array,
    offset,
    numBytes
...
ZXing.Common.BitArray.prototype.reverse = function() {
    ...
    if (this.size != oldBitsLen * 32) {
        ...
        var mask = 1;
        for (var i1 = 0; i1 < 31 - leftOffset; i1++) mask = (mask << 1) | 1;
...
ZXing.Common.BitArray.prototype.Equals = function(o) {
    ...
    for (var index = 0; index < this.size; index++) {
        if (this.bits[index] != other.bits[index]) return false;
    }
    return true;
};
...
ZXing.Common.BitMatrix = function(width, height, rowSize, bits) {
    ...
    this.bits =
        arguments.length == 4
            ...
            : arguments.length == 3
            ? rowSize
            : new Int32Array(this.rowSize * this.height);
};
...
ZXing.Common.BitMatrix.parse = function(
    ...
    while (pos < stringRepresentation.length) {
        if (
            stringRepresentation.substr(pos, 1) == "\n" ||
            ...
            stringRepresentation.substr(pos, setString.length) == setString
        ) {
            pos += setString.length;
            bits[bitsPos] = true;
            bitsPos++;
        } else if (
            stringRepresentation.substr(pos, unsetString.length) == unsetString
        ) {
            ...
            throw new Error(
                "illegal character encountered: " +
                    stringRepresentation.substr(pos)
            );
        }
    }
    if (bitsPos > rowStartPos) {
...
ZXing.Common.BitMatrix.prototype.xor = function(mask) {
    if (
        this.width != mask.get_Width() ||
        this.height != mask.get_Height() ||
        this.rowSize != mask.get_RowSize()
    ) {
...
ZXing.Common.BitMatrix.prototype.setRegion = function(
    left,
    top,
    width,
    height
...
ZXing.Common.BitMatrix.prototype.Equals = function(obj) {
    ...
    var other =
        obj instanceof ZXing.Common.BitMatrix || obj == null
            ? obj
            : (function() {
...
ZXing.Common.BitMatrix.prototype.Clone = function() {
    return new ZXing.Common.BitMatrix(
        this.width,
        this.height,
        this.rowSize,
        this.bits.slice(0)
...
ZXing.Common.CharacterSetECI.addCharacterSet = function(value, encodingNames) {
    if (Array.isArray(encodingNames)) {
        ...
        for (
            var $i4 = 0, $l4 = encodingNames.length, t = encodingNames[$i4];
            $i4 < $l4;
            $i4++, t = encodingNames[$i4]
        ) {
...
ZXing.Common.CharacterSetECI.addCharacterSet(12, [
    "ISO-8859-4",
    "ISO-8859-10",
    "ISO8859_10"
]);
...
ZXing.Common.CharacterSetECI.addCharacterSet(16, [
    "ISO-8859-1",
    "ISO-8859-14",
    "ISO8859_14"
]);
ZXing.Common.CharacterSetECI.addCharacterSet(17, ["ISO-8859-15", "ISO8859_15"]);
ZXing.Common.CharacterSetECI.addCharacterSet(18, [
    "ISO-8859-3",
    "ISO-8859-16",
    "ISO8859_16"
]);
...
ZXing.Common.CharacterSetECI.addCharacterSet(29, [
    "GB18030",
    "GB2312",
    "EUC_CN",
    "GBK"
...
ZXing.Common.GlobalHistogramBinarizer.prototype.getBlackRow = function(y, row) {
    ...
    if (
        !function() {
            ...
            var $res = ZXing.Common.GlobalHistogramBinarizer.estimateBlackPoint(
                localBuckets,
                $1
            );
            blackPoint = $1.Value;
...
ZXing.Common.GlobalHistogramBinarizer.prototype.createBinarizer = function(
    source
) {
    return new ZXing.Common.GlobalHistogramBinarizer(source);
};
ZXing.Common.GlobalHistogramBinarizer.prototype.initArrays = function(
    luminanceSize
) {
...
ZXing.Common.GlobalHistogramBinarizer.estimateBlackPoint = function(
    buckets,
    blackPoint
) {
    blackPoint.Value = 0;
...
ZXing.Common.HybridBinarizer.prototype.binarizeEntireImage = function() {
    if (this.matrix == null) {
        ...
        if (width >= 40 && height >= 40) {
            ...
            var blackPoints = ZXing.Common.HybridBinarizer.calculateBlackPoints(
                luminances,
                subWidth,
                subHeight,
                width,
            ...
            ZXing.Common.HybridBinarizer.calculateThresholdForBlock(
                luminances,
                subWidth,
                subHeight,
                width,
            ...
            this.matrix = ZXing.Common.GlobalHistogramBinarizer.prototype.get_BlackMatrix.call(
                this
            );
        }
    }
};
ZXing.Common.HybridBinarizer.calculateThresholdForBlock = function(
    luminances,
    subWidth,
    subHeight,
    width,
    ...
    for (var y = 0; y < subHeight; y++) {
        ...
        for (var x = 0; x < subWidth; x++) {
            ...
            ZXing.Common.HybridBinarizer.thresholdBlock(
                luminances,
                xoffset,
                yoffset,
                average,
...
ZXing.Common.HybridBinarizer.thresholdBlock = function(
    luminances,
    xoffset,
    yoffset,
    threshold,
...
ZXing.Common.HybridBinarizer.calculateBlackPoints = function(
    luminances,
    subWidth,
    subHeight,
    width,
    ...
    for (var y = 0; y < subHeight; y++) {
        ...
        for (var x = 0; x < subWidth; x++) {
            ...
            var max = 0;
            for (
                var yy = 0, offset = yoffset * width + xoffset;
                yy < 8;
                yy++, offset += width
            ) {
...
ZXing.PDF417.PDF417Common.MAX_CODEWORDS_IN_BARCODE =
    ZXing.PDF417.PDF417Common.NUMBER_OF_CODEWORDS - 1;
...
ZXing.PDF417.PDF417Common.SYMBOL_TABLE = [
    66142,
    66170,
    66206,
    66236,
...
ZXing.PDF417.PDF417Common.CODEWORD_TABLE = [
    2627,
    1819,
    2622,
    2621,
...
ZXing.PDF417.PDF417Common.getBitCountSum = function(moduleBitCount) {
    ...
    for (
        var $i16 = 0,
            $l16 = moduleBitCount.length,
            count = moduleBitCount[$i16];
        $i16 < $l16;
        $i16++, count = moduleBitCount[$i16]
    ) {
        bitCountSum += count;
    }
    return bitCountSum;
...
ZXing.PDF417.PDF417Common.getCodeword = function(symbol) {
    ...
    return (
        (ZXing.PDF417.PDF417Common.CODEWORD_TABLE[i] - 1) %
        ZXing.PDF417.PDF417Common.NUMBER_OF_CODEWORDS
    );
};
...
ZXing.PDF417.Internal.Detector.INDEXES_START_PATTERN = new Int32Array([
    0,
    4,
    1,
    5
]);
ZXing.PDF417.Internal.Detector.INDEXES_STOP_PATTERN = new Int32Array([
    6,
    2,
    7,
    3
...
ZXing.PDF417.Internal.Detector.START_PATTERN = new Int32Array([
    8,
    1,
    1,
    1,
...
ZXing.PDF417.Internal.Detector.STOP_PATTERN = new Int32Array([
    7,
    1,
    1,
    3,
...
ZXing.PDF417.Internal.Detector.detectSingle = function(image, hints, multiple) {
    ...
    var barcodeCoordinates = ZXing.PDF417.Internal.Detector.detectMultiple(
        multiple,
        bitMatrix
    );
    if (barcodeCoordinates == null || barcodeCoordinates.length == 0) {
        //bitMatrix = $.extend({}, bitMatrix, true);
        bitMatrix = new ZXing.Common.BitMatrix(
            bitMatrix.width,
            bitMatrix.height,
            bitMatrix.rowSize,
            JSON.parse(JSON.stringify(bitMatrix.bits))
        );
        bitMatrix.rotate180();
        barcodeCoordinates = ZXing.PDF417.Internal.Detector.detectMultiple(
            multiple,
            bitMatrix
        );
    }
    return new ZXing.PDF417.Internal.PDF417DetectorResult(
        bitMatrix,
        barcodeCoordinates
    );
};
ZXing.PDF417.Internal.Detector.detectMultiple = function(multiple, bitMatrix) {
    ...
    while (row < bitMatrix.height) {
        var vertices = ZXing.PDF417.Internal.Detector.findVertices(
            bitMatrix,
            row,
            column
        );
...
ZXing.PDF417.Internal.Detector.findVertices = function(
    matrix,
    startRow,
    startColumn
) {
    ...
    ZXing.PDF417.Internal.Detector.copyToResult(
        result,
        ZXing.PDF417.Internal.Detector.findRowsWithPattern(
            matrix,
            height,
            width,
            startRow,
            startColumn,
            ZXing.PDF417.Internal.Detector.START_PATTERN
        ),
        ZXing.PDF417.Internal.Detector.INDEXES_START_PATTERN
    );
    if (result[4] != null) {
    ...
    ZXing.PDF417.Internal.Detector.copyToResult(
        ...
        ZXing.PDF417.Internal.Detector.findRowsWithPattern(
            ...
            startRow,
            startColumn,
            ZXing.PDF417.Internal.Detector.STOP_PATTERN
        ),
        ZXing.PDF417.Internal.Detector.INDEXES_STOP_PATTERN
    );
    return result;
};
ZXing.PDF417.Internal.Detector.copyToResult = function(
    result,
    tmpResult,
    destinationIndexes
) {
...
ZXing.PDF417.Internal.Detector.findRowsWithPattern = function(
    matrix,
    height,
    width,
    startRow,
    ...
    for (; startRow < height; startRow += 5) {
        var loc = ZXing.PDF417.Internal.Detector.findGuardPattern(
            matrix,
            startColumn,
            startRow,
            width,
        ...
        if (loc != null) {
            while (startRow > 0) {
                var previousRowLoc = ZXing.PDF417.Internal.Detector.findGuardPattern(
                    matrix,
                    startColumn,
                    --startRow,
                    width,
    ...
    if (found) {
        ...
        for (; stopRow < height; stopRow++) {
            loc = ZXing.PDF417.Internal.Detector.findGuardPattern(
                matrix,
                previousRowLoc[0],
                stopRow,
                width,
            ...
            if (
                loc != null &&
                Math.abs(previousRowLoc[0] - loc[0]) < 5 &&
                Math.abs(previousRowLoc[1] - loc[1]) < 5
            ) {
                previousRowLoc = loc;
                skippedRowCount = 0;
            } else {
...
ZXing.PDF417.Internal.Detector.findGuardPattern = function(
    matrix,
    column,
    row,
    width,
    ...
    var pixelDrift = 0;
    while (
        matrix.get_Item(patternStart, row) &&
        patternStart > 0 &&
        pixelDrift++ < 3
    ) {
        patternStart--;
    ...
    for (; x < width; x++) {
        ...
        if (pixel ^ isWhite) {
            ...
            if (counterPosition == patternLength - 1) {
                if (
                    ZXing.PDF417.Internal.Detector.patternMatchVariance(
                        counters,
                        pattern,
                        204
                    ) < 107
...
ZXing.PDF417.Internal.Detector.patternMatchVariance = function(
    counters,
    pattern,
...
ZXing.PDF417.Internal.EC.ModulusPoly.prototype.getCoefficient = function(
    degree
) {
...
ZXing.PDF417.Internal.EC.ModulusPoly.prototype.evaluateAt = function(a) {
    ...
    if (a == 1) {
        for (
            var $i11 = 0,
                $t11 = this.coefficients,
                $l11 = $t11.length,
                coefficient = $t11[$i11];
            $i11 < $l11;
            $i11++, coefficient = $t11[$i11]
        ) {
...
ZXing.PDF417.Internal.EC.ModulusPoly.prototype.getNegative = function() {
    ...
    return new ZXing.PDF417.Internal.EC.ModulusPoly(
        this.field,
...
ZXing.PDF417.Internal.EC.ModulusPoly.prototype.multiplyByMonomial = function(
    degree,
    coefficient
) {
    if (degree < 0) {
...
ZXing.PDF417.Internal.EC.ModulusPoly.prototype.divide = function(other) {
    ...
    while (
        remainder.get_Degree() >= other.get_Degree() &&
        !remainder.get_isZero()
    ) {
        var degreeDifference = remainder.get_Degree() - other.get_Degree();
        var scale = this.field.multiply(
            remainder.getCoefficient(remainder.get_Degree()),
...
ZXing.PDF417.Internal.EC.ModulusGF = function(modulus, generator) {
    ...
    this.Zero = new ZXing.PDF417.Internal.EC.ModulusPoly(
        this,
        new Int32Array([0])
    );
    this.One = new ZXing.PDF417.Internal.EC.ModulusPoly(
        this,
        new Int32Array([1])
    );
};
ZXing.PDF417.Internal.EC.ModulusGF.PDF417_GF = new ZXing.PDF417.Internal.EC.ModulusGF(
    ZXing.PDF417.PDF417Common.NUMBER_OF_CODEWORDS,
    3
);
ZXing.PDF417.Internal.EC.ModulusGF.prototype.buildMonomial = function(
    degree,
    coefficient
) {
    if (degree < 0) {
...
ZXing.PDF417.Internal.EC.ModulusGF.prototype.multiply = function(a, b) {
    ...
    return this.expTable[
        (this.logTable[a] + this.logTable[b]) % (this.modulus - 1)
    ];
};
...
ZXing.PDF417.Internal.EC.ErrorCorrection.prototype.decode = function(
    received,
    numECCodewords,
    erasures,
    errorLocationsCount
    ...
    if (erasures != null) {
        for (
            var $i10 = 0, $l10 = erasures.length, erasure = erasures[$i10];
            $i10 < $l10;
            $i10++, erasure = erasures[$i10]
        ) {
            var b = this.field.exp(received.length - 1 - erasure);
            var term = new ZXing.PDF417.Internal.EC.ModulusPoly(
                this.field,
                new Int32Array([this.field.subtract(0, b), 1])
            );
    ...
    var sigmaOmega = this.runEuclideanAlgorithm(
        this.field.buildMonomial(numECCodewords, 1),
        syndrome,
        numECCodewords
    );
    if (sigmaOmega == null) {
    ...
    for (i = 0; i < errorLocations.length; i++) {
        ...
        received[position] = this.field.subtract(
            received[position],
            errorMagnitudes[i]
        );
    }
...
ZXing.PDF417.Internal.EC.ErrorCorrection.prototype.runEuclideanAlgorithm = function(
    a,
    b,
    R
) {
...
ZXing.PDF417.Internal.EC.ErrorCorrection.prototype.findErrorLocations = function(
    errorLocator
) {
...
ZXing.PDF417.Internal.EC.ErrorCorrection.prototype.findErrorMagnitudes = function(
    errorEvaluator,
    errorLocator,
    errorLocations
) {
    ...
    var formalDerivative = new ZXing.PDF417.Internal.EC.ModulusPoly(
        this.field,
        formalDerivativeCoefficients
    );
    ...
    for (i = 0; i < s; i++) {
        ...
        var numerator = this.field.subtract(
            0,
            errorEvaluator.evaluateAt(xiInverse)
        );
        var denominator = this.field.inverse(
            formalDerivative.evaluateAt(xiInverse)
        );
...
ZXing.PDF417.Internal.BarcodeMetadata = function(
    columnCount,
    rowCountUpperPart,
    rowCountLowerPart,
...
ZXing.PDF417.Internal.BarcodeValue.prototype.getConfidence = function(
    barcodeValue
) {
    return typeof this.values[barcodeValue] != "undefined"
        ? this.values[barcodeValue]
        : 0;
};
...
ZXing.PDF417.Internal.BoundingBox = function(
    image,
    topLeft,
    bottomLeft,
    topRight,
...
ZXing.PDF417.Internal.BoundingBox.Create = function(
    image,
    topLeft,
    bottomLeft,
    topRight,
    ...
    if (arguments.length > 1) {
        ...
        return new ZXing.PDF417.Internal.BoundingBox(
            image,
            topLeft,
            bottomLeft,
            topRight,
...
ZXing.PDF417.Internal.BoundingBox.merge = function(leftBox, rightBox) {
    if (!leftBox) return rightBox;
    if (!rightBox) return leftBox;
    return new ZXing.PDF417.Internal.BoundingBox(
        leftBox.image,
        leftBox.TopLeft,
        leftBox.BottomLeft,
        rightBox.TopRight,
        rightBox.BottomRight
    );
};
ZXing.PDF417.Internal.BoundingBox.prototype.addMissingRows = function(
    missingStartRows,
    missingEndRows,
    isLeft
) {
...
ZXing.PDF417.Internal.BoundingBox.prototype.calculateMinMaxValues = function() {
    if (!this.TopLeft) {
        ...
        this.TopRight = new ZXing.ResultPoint(
            this.image.width - 1,
            this.TopLeft.y
        );
        this.BottomRight = new ZXing.ResultPoint(
            this.image.width - 1,
            this.TopLeft.y
        );
    }
...
ZXing.PDF417.Internal.BoundingBox.prototype.SetBottomRight = function(
    bottomRight
) {
    this.BottomRight = bottomRight;
    this.calculateMinMaxValues();
...
ZXing.PDF417.Internal.Codeword.prototype.IsValidRowNumber = function(
    rowNumber
) {
    rowNumber = parseInt(rowNumber);
    return (
        rowNumber != ZXing.PDF417.Internal.Codeword.BARCODE_ROW_UNKNOWN &&
        this.Bucket == (rowNumber % 3) * 3
    );
};
ZXing.PDF417.Internal.Codeword.prototype.setRowNumberAsRowIndicatorColumn = function() {
    this.RowNumber =
        Math.floor(this.Value / 30) * 3 + Math.floor(this.Bucket / 3);
};
...
ZXing.PDF417.Internal.DecodedBitStreamParser.PUNCT_CHARS = [
    ";",
    "<",
    ">",
    "@",
...
ZXing.PDF417.Internal.DecodedBitStreamParser.MIXED_CHARS = [
    "0",
    "1",
    "2",
    "3",
...
for (
    var i = 2;
    i < ZXing.PDF417.Internal.DecodedBitStreamParser.EXP900.length;
    i++
) {
    ZXing.PDF417.Internal.DecodedBitStreamParser.EXP900[
        i
    ] = ZXing.PDF417.Internal.DecodedBitStreamParser.EXP900[i - 1].multiply(
        nineHundred
    );
}
ZXing.PDF417.Internal.DecodedBitStreamParser.decode = function(
    codewords,
    ecLevel
) {
    var result = "";
    ...
    while (codeIndex < codewords[0]) {
        switch (code) {
            case 900:
                ...
                codeIndex = ZXing.PDF417.Internal.DecodedBitStreamParser.textCompaction(
                    p1
                );
                result = p1.result;
                break;
            ...
            case 924:
                ...
                codeIndex = ZXing.PDF417.Internal.DecodedBitStreamParser.byteCompaction(
                    p1
                );
                result = p1.result;
                break;
            ...
            case 902:
                ...
                codeIndex = ZXing.PDF417.Internal.DecodedBitStreamParser.numericCompaction(
                    p1
                );
                result = p1.result;
                break;
            case 927:
                var charsetECI = ZXing.Common.CharacterSetECI.getCharacterSetECIByValue(
                    codewords[codeIndex++]
                );
                break;
            case 926:
            ...
            case 925:
                ...
                break;
            case 928:
                codeIndex = ZXing.PDF417.Internal.DecodedBitStreamParser.decodeMacroBlock(
                    codewords,
                    codeIndex,
                    resultMetadata
                );
    ...
    var decoderResult = new ZXing.Common.DecoderResult(
        null,
        result.toString(),
        null,
        ecLevel
...
ZXing.PDF417.Internal.DecodedBitStreamParser.decodeMacroBlock = function(
    codewords,
    codeIndex,
    resultMetadata
) {
    ...
    var s = ZXing.PDF417.Internal.DecodedBitStreamParser.decodeBase900toBase10(
        segmentIndexArray,
        2
    );
    if (!s) return -1;
    ...
    if (codewords[codeIndex] == 923) {
        codeIndex++;
        var additionalOptionCodeWords = new Int32Array(
            codewords[0] - codeIndex
        );
...
ZXing.PDF417.Internal.DecodedBitStreamParser.textCompaction = function(p) {
    ...
    var p1 = {
        textCompactionData: textCompactionData,
        byteCompactionData: byteCompactionData,
        length: index,
        result: result
    };
...
ZXing.PDF417.Internal.DecodedBitStreamParser.decodeTextCompaction = function(
    p
) {
    var textCompactionData = p.textCompactionData,
        byteCompactionData = p.byteCompactionData,
        length = p.length;
    var subMode = 0;
...
ZXing.PDF417.Internal.DecodedBitStreamParser.decodeBase900toBase10 = function(
    codewords,
    count
) {
...
ZXing.PDF417.Internal.DetectionResult.prototype.getDetectionResultColumns = function() {
    ...
    this.adjustIndicatorColumnRowNumbers(
        this.DetectionResultColumns[this.ColumnCount + 1]
    );
    var unadjustedCodewordCount =
        ZXing.PDF417.PDF417Common.MAX_CODEWORDS_IN_BARCODE;
    ...
    do {
        ...
        unadjustedCodewordCount > 0 &&
        unadjustedCodewordCount < previousUnadjustedCount
    );
    return this.DetectionResultColumns;
};
ZXing.PDF417.Internal.DetectionResult.prototype.adjustIndicatorColumnRowNumbers = function(
    detectionResultColumn
) {
    if (detectionResultColumn) {
        (detectionResultColumn instanceof
...
ZXing.PDF417.Internal.DetectionResult.prototype.adjustRowNumbersMy = function() {
    ...
    for (
        var barcodeColumn = 1;
        barcodeColumn < this.ColumnCount + 1;
        barcodeColumn++
    ) {
...
ZXing.PDF417.Internal.DetectionResult.prototype.adjustRowNumbers = function(
    barcodeColumn,
    codewordsRow,
    codewords
) {
    ...
    var previousColumnCodewords = this.DetectionResultColumns[barcodeColumn - 1]
        .Codewords;
    var nextColumnCodewords = previousColumnCodewords;
    if (this.DetectionResultColumns[barcodeColumn + 1] != null) {
        nextColumnCodewords = this.DetectionResultColumns[barcodeColumn + 1]
            .Codewords;
    }
    ...
    for (
        var $i5 = 0,
            $l5 = otherCodewords.length,
            otherCodeword = otherCodewords[$i5];
        $i5 < $l5;
        $i5++, otherCodeword = otherCodewords[$i5]
    ) {
        if (
            ZXing.PDF417.Internal.DetectionResult.adjustRowNumber(
                codeword,
                otherCodeword
            )
        ) {
...
ZXing.PDF417.Internal.DetectionResult.prototype.adjustRowNumbersFromBothRI = function() {
    if (
        ...
        this.DetectionResultColumns[this.ColumnCount + 1] == null
    ) {
        return;
    }
    var LRIcodewords = this.DetectionResultColumns[0].Codewords;
    var RRIcodewords = this.DetectionResultColumns[this.ColumnCount + 1]
        .Codewords;
    for (
        var codewordsRow = 0;
        ...
        if (
            LRIcodewords[codewordsRow] != null &&
            ...
            for (
                var barcodeColumn = 1;
                barcodeColumn <= this.ColumnCount;
                barcodeColumn++
            ) {
                var codeword = this.DetectionResultColumns[barcodeColumn]
                ...
                if (!codeword.get_HasValidRowNumber()) {
                    this.DetectionResultColumns[barcodeColumn].Codewords[
                        codewordsRow
                    ] = null;
                }
            }
...
ZXing.PDF417.Internal.DetectionResult.prototype.adjustRowNumbersFromRRI = function() {
    ...
    for (
        ...
        for (
            var barcodeColumn = this.ColumnCount + 1;
            barcodeColumn > 0 && invalidRowCounts < 2;
            barcodeColumn--
        ) {
            var codeword = this.DetectionResultColumns[barcodeColumn].Codewords[
                codewordsRow
            ];
            if (codeword != null) {
                invalidRowCounts = ZXing.PDF417.Internal.DetectionResult.adjustRowNumberIfValid(
...
ZXing.PDF417.Internal.DetectionResult.prototype.adjustRowNumbersFromLRI = function() {
    ...
    for (
        ...
        for (
            var barcodeColumn = 1;
            barcodeColumn < this.ColumnCount + 1 && invalidRowCounts < 2;
            barcodeColumn++
        ) {
            var codeword = this.DetectionResultColumns[barcodeColumn].Codewords[
                codewordsRow
...
ZXing.PDF417.Internal.DetectionResult.adjustRowNumberIfValid = function(
    rowIndicatorRowNumber,
    invalidRowCounts,
    codeword
) {
...
ZXing.PDF417.Internal.DetectionResult.adjustRowNumber = function(
    codeword,
    otherCodeword
) {
    ...
    if (
        otherCodeword.get_HasValidRowNumber() &&
...
ZXing.PDF417.Internal.DetectionResult.prototype.toString = function() {
    ...
    for (
        ...
        for (
            var barcodeColumn = 0;
            barcodeColumn < this.ColumnCount + 2;
            barcodeColumn++
        ) {
            ...
            formatter += " {0}|{1}".format(
                FormatInteger(codeword.RowNumber, 3),
                FormatInteger(codeword.Value, 3)
            );
        }
        formatter += "\n";
    }
...
ZXing.PDF417.Internal.DetectionResultColumn.prototype.IndexForRow = function(
    imageRow
) {
    return imageRow - this.Box.MinY;
};
ZXing.PDF417.Internal.DetectionResultColumn.prototype.RowForIndex = function(
    codewordIndex
) {
    return this.Box.MinY + codewordIndex;
};
ZXing.PDF417.Internal.DetectionResultColumn.prototype.getCodeword = function(
    imageRow
) {
    return this.Codewords[this.imageRowToCodewordIndex(imageRow)];
};
ZXing.PDF417.Internal.DetectionResultColumn.prototype.getCodewordNearby = function(
    imageRow
) {
...
ZXing.PDF417.Internal.DetectionResultColumn.prototype.imageRowToCodewordIndex = function(
    imageRow
) {
    return imageRow - this.Box.MinY;
};
ZXing.PDF417.Internal.DetectionResultColumn.prototype.setCodeword = function(
    imageRow,
    codeword
) {
...
ZXing.PDF417.Internal.DetectionResultColumn.prototype.toString = function() {
    ...
    var row = 0;
    for (
        var $i6 = 0, $t6 = this.Codewords, $l6 = $t6.length, cw = $t6[$i6];
        $i6 < $l6;
        $i6++, cw = $t6[$i6]
    ) {
        if (cw == null) {
            ...
            builder += "{0}: {1}|{2}\n".format(
                FormatInteger(row++, 3),
                FormatInteger(cw.RowNumber, 3),
                FormatInteger(cw.Value, 3)
            );
        }
    }
    return builder;
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn = function(
    box,
    isLeft
) {
    this.IsLeft = false;
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.setRowNumbers = function() {
    for (
        var $i7 = 0, $t7 = this.Codewords, $l7 = $t7.length, cw = $t7[$i7];
        $i7 < $l7;
        $i7++, cw = $t7[$i7]
    ) {
        if (cw != null) {
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.adjustCompleteIndicatorColumnRowNumbers = function(
    metadata
) {
    ...
    for (var codewordRow = firstRow; codewordRow < lastRow; codewordRow++) {
        ...
        if (rowDifference == 0) {
            ...
            for (
                var i = 1;
                i <= checkedRows && !closePreviousCodewordFound;
                i++
            ) {
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.adjustIncompleteIndicatorColumnRowNumbers = function(
    metadata
) {
    var top = this.IsLeft ? this.Box.TopLeft : this.Box.TopRight;
    var bottom = this.IsLeft ? this.Box.BottomLeft : this.Box.BottomRight;
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.getBarcodeMetadata = function() {
    if (!this.barcodeMetaData) {
        ...
        for (
            var $i9 = 0, $l9 = codewords.length, codeword = codewords[$i9];
            $i9 < $l9;
            $i9++, codeword = codewords[$i9]
        ) {
            if (codeword == null) {
                continue;
            }
            ...
            switch (codewordRowNumber % 3) {
                case 0:
                    barcodeRowCountUpperPart.setValue(
                        rowIndicatorValue * 3 + 1
                    );
                    break;
                case 1:
        ...
        if (
            barcodeColumnCountValues.length == 0 ||
            ...
            barcodeRowCountUpperPartValues[0] +
                ...
                ZXing.PDF417.PDF417Common.MIN_ROWS_IN_BARCODE ||
            barcodeRowCountUpperPartValues[0] +
                barcodeRowCountLowerPartValues[0] >
                ZXing.PDF417.PDF417Common.MAX_ROWS_IN_BARCODE
        ) {
            return null;
        }
        var barcodeMetadata = new ZXing.PDF417.Internal.BarcodeMetadata(
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.removeIncorrectCodewords = function(
    codewords,
    metadata
) {
    for (var row = 0; row < codewords.length; row++) {
        ...
        switch (rowNumber % 3) {
            ...
            case 1:
                if (
                    indicatorValue % 3 != metadata.RowCountLower ||
...
ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn.prototype.toString = function() {
    return (
        ...
        this.IsLeft +
        " \n" +
        ZXing.PDF417.Internal.DetectionResultColumn.prototype.toString.call(
            this
        )
    );
};
...
ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE = new Array(
    ZXing.PDF417.PDF417Common.SYMBOL_TABLE.length
);
for (
    var s = 0;
    s < ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE.length;
    s++
) {
    ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE[
        s
    ] = new Float32Array(ZXing.PDF417.PDF417Common.BARS_IN_MODULE);
}
for (var i = 0; i < ZXing.PDF417.PDF417Common.SYMBOL_TABLE.length; i++) {
    ...
    for (var j = 0; j < ZXing.PDF417.PDF417Common.BARS_IN_MODULE; j++) {
        ...
        ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE[i][
            ZXing.PDF417.PDF417Common.BARS_IN_MODULE - j - 1
        ] = size / ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD;
    }
}
ZXing.PDF417.Internal.PDF417CodewordDecoder.getDecodedValue = function(
    moduleBitCount
) {
    var decodedValue = ZXing.PDF417.Internal.PDF417CodewordDecoder.getDecodedCodewordValue(
        ZXing.PDF417.Internal.PDF417CodewordDecoder.sampleBitCounts(
            moduleBitCount
        )
    );
    ...
    return ZXing.PDF417.Internal.PDF417CodewordDecoder.getClosestDecodedValue(
        moduleBitCount
    );
};
ZXing.PDF417.Internal.PDF417CodewordDecoder.sampleBitCounts = function(
    moduleBitCount
) {
    ...
    for (var i = 0; i < ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD; i++) {
        var sampleIndex =
            bitCountSum / (2 * ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD) +
            (i * bitCountSum) / ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD;
...
ZXing.PDF417.Internal.PDF417CodewordDecoder.getDecodedCodewordValue = function(
    moduleBitCount
) {
    var decodedValue = ZXing.PDF417.Internal.PDF417CodewordDecoder.getBitValue(
        moduleBitCount
    );
    return ZXing.PDF417.PDF417Common.getCodeword(decodedValue) ==
        ZXing.PDF417.PDF417Common.INVALID_CODEWORD
        ? ZXing.PDF417.PDF417Common.INVALID_CODEWORD
        : decodedValue;
};
ZXing.PDF417.Internal.PDF417CodewordDecoder.getBitValue = function(
    moduleBitCount
) {
    var result = 0;
...
ZXing.PDF417.Internal.PDF417CodewordDecoder.getClosestDecodedValue = function(
    moduleBitCount
) {
    var bitCountSum = ZXing.PDF417.PDF417Common.getBitCountSum(moduleBitCount);
    var bitCountRatios = new Float32Array(
        ZXing.PDF417.PDF417Common.BARS_IN_MODULE
    );
    for (var i = 0; i < bitCountRatios.length; i++) {
        bitCountRatios[i] = moduleBitCount[i] / bitCountSum;
    }
    var bestMatchError = 3.402823e38;
    var bestMatch = ZXing.PDF417.PDF417Common.INVALID_CODEWORD;
    for (
        var j = 0;
        j < ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE.length;
        j++
    ) {
        var error = 0;
        var ratioTableRow =
            ZXing.PDF417.Internal.PDF417CodewordDecoder.RATIOS_TABLE[j];
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.decode = function(
    image,
    imageTopLeft,
    imageBottomLeft,
    imageTopRight,
    ...
    var boundingBox = ZXing.PDF417.Internal.BoundingBox.Create(
        image,
        imageTopLeft,
        imageBottomLeft,
        imageTopRight,
    ...
    for (var i = 0; i < 2; i++) {
        if (imageTopLeft != null) {
            leftRowIndicatorColumn = ZXing.PDF417.Internal.PDF417ScanningDecoder.getRowIndicatorColumn(
                image,
                boundingBox,
                imageTopLeft,
                true,
        ...
        if (imageTopRight != null) {
            rightRowIndicatorColumn = ZXing.PDF417.Internal.PDF417ScanningDecoder.getRowIndicatorColumn(
                image,
                boundingBox,
                imageTopRight,
                false,
        ...
        detectionResult = ZXing.PDF417.Internal.PDF417ScanningDecoder.merge(
        ...
        if (
            ...
            (detectionResult.Box.MinY < boundingBox.MinY ||
                detectionResult.Box.MaxY > boundingBox.MaxY)
        ) {
    ...
    detectionResult.DetectionResultColumns[
        maxBarcodeColumn
    ] = rightRowIndicatorColumn;
    ...
    for (
        ...
        if (barcodeColumn == 0 || barcodeColumn == maxBarcodeColumn) {
            detectionResultColumn = new ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn(
                boundingBox,
                barcodeColumn == 0
            );
        } else {
            detectionResultColumn = new ZXing.PDF417.Internal.DetectionResultColumn(
                boundingBox
            );
        }
        detectionResult.DetectionResultColumns[
        ...
        for (
            ...
            startColumn = ZXing.PDF417.Internal.PDF417ScanningDecoder.getStartColumn(
                detectionResult,
                barcodeColumn,
                imageRow,
                leftToRight
            ...
            var codeword = ZXing.PDF417.Internal.PDF417ScanningDecoder.detectCodeword(
                image,
                boundingBox.MinX,
                boundingBox.MaxX,
                leftToRight,
            ...
            if (codeword != null) {
                ...
                minCodewordWidth = Math.min(
                    minCodewordWidth,
                    codeword.get_Width()
                );
                maxCodewordWidth = Math.max(
                    maxCodewordWidth,
                    codeword.get_Width()
                );
            }
    ...
    return ZXing.PDF417.Internal.PDF417ScanningDecoder.createDecoderResult(
        detectionResult
    );
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.merge = function(
    leftRowIndicatorColumn,
    rightRowIndicatorColumn
) {
    ...
    var barcodeMetadata = ZXing.PDF417.Internal.PDF417ScanningDecoder.getBarcodeMetadata(
        leftRowIndicatorColumn,
        rightRowIndicatorColumn
    );
    if (barcodeMetadata == null) {
    ...
    var boundingBox = ZXing.PDF417.Internal.BoundingBox.merge(
        ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustBoundingBox(
            leftRowIndicatorColumn
        ),
        ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustBoundingBox(
            rightRowIndicatorColumn
    ...
    return new ZXing.PDF417.Internal.DetectionResult(
        barcodeMetadata,
        boundingBox
    );
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustBoundingBox = function(
    rowIndicatorColumn
) {
    if (rowIndicatorColumn == null) {
        return null;
    ...
    var maxRowHeight = ZXing.PDF417.Internal.PDF417ScanningDecoder.getMax(
        rowHeights
    );
    var missingStartRows = 0;
    for (
        var $i12 = 0, $l12 = rowHeights.length, rowHeight = rowHeights[$i12];
        $i12 < $l12;
        $i12++, rowHeight = rowHeights[$i12]
    ) {
    ...
    return rowIndicatorColumn.Box.addMissingRows(
        missingStartRows,
        missingEndRows,
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.getBarcodeMetadata = function(
    leftRowIndicatorColumn,
    rightRowIndicatorColumn
) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.getRowIndicatorColumn = function(
    image,
    boundingBox,
    startPoint,
    leftToRight,
    ...
    var rowIndicatorColumn = new ZXing.PDF417.Internal.DetectionResultRowIndicatorColumn(
        boundingBox,
        leftToRight
    );
    for (var i = 0; i < 2; i++) {
        ...
        for (
            var imageRow = startPoint.y;
            imageRow <= boundingBox.MaxY && imageRow >= boundingBox.MinY;
            imageRow += increment
        ) {
            var codeword = ZXing.PDF417.Internal.PDF417ScanningDecoder.detectCodeword(
                image,
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustCodewordCount = function(
    detectionResult,
    barcodeMatrix
) {
    var numberOfCodewords = barcodeMatrix[0][1].getValue();
    var calculatedNumberOfCodewords =
        detectionResult.ColumnCount * detectionResult.get_RowCount() -
        ZXing.PDF417.Internal.PDF417ScanningDecoder.getNumberOfECCodeWords(
            detectionResult.get_ErrorCorrectionLevel()
        );
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.createDecoderResult = function(
    detectionResult
) {
    var barcodeMatrix = ZXing.PDF417.Internal.PDF417ScanningDecoder.createBarcodeMatrix(
        detectionResult
    );
    if (barcodeMatrix == null) return null;
    if (
        !ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustCodewordCount(
            detectionResult,
            barcodeMatrix
        )
    ) {
    ...
    var erasures = [];
    var codewords = new Int32Array(
        detectionResult.get_RowCount() * detectionResult.ColumnCount
    );
    ...
    return ZXing.PDF417.Internal.PDF417ScanningDecoder.createDecoderResultFromAmbiguousValues(
        detectionResult.get_ErrorCorrectionLevel(),
        codewords,
        erasures,
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.createDecoderResultFromAmbiguousValues = function(
    ecLevel,
    codewords,
    erasureArray,
    ambiguousIndexes,
    ...
    while (tries-- > 0) {
        ...
        try {
            var result = ZXing.PDF417.Internal.PDF417ScanningDecoder.decodeCodewords(
                codewords,
                ecLevel,
                erasureArray
            );
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.createBarcodeMatrix = function(
    detectionResult
) {
    ...
    for (
        var $i13 = 0,
            $t13 = detectionResult.getDetectionResultColumns(),
            $l13 = $t13.length,
            detectionResultColumn = $t13[$i13];
        $i13 < $l13;
        $i13++, detectionResultColumn = $t13[$i13]
    ) {
        if (detectionResultColumn != null) {
            for (
                var $i14 = 0,
                    $t14 = detectionResultColumn.Codewords,
                    $l14 = $t14.length,
                    codeword = $t14[$i14];
                $i14 < $l14;
                $i14++, codeword = $t14[$i14]
            ) {
                if (codeword != null) {
                    var rowNumber = codeword.RowNumber;
                    if (rowNumber >= 0) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.isValidBarcodeColumn = function(
    detectionResult,
    barcodeColumn
) {
    return (
        barcodeColumn >= 0 &&
        barcodeColumn < detectionResult.DetectionResultColumns.length
    );
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.getStartColumn = function(
    detectionResult,
    barcodeColumn,
    imageRow,
    leftToRight
    ...
    var codeword = null;
    if (
        ZXing.PDF417.Internal.PDF417ScanningDecoder.isValidBarcodeColumn(
            detectionResult,
            barcodeColumn - offset
        )
    ) {
        codeword = detectionResult.DetectionResultColumns[
            barcodeColumn - offset
    ...
    while (
        ...
        for (
            var $i15 = 0,
                $t15 =
                    detectionResult.DetectionResultColumns[barcodeColumn]
                        .Codewords,
                $l15 = $t15.length,
                previousRowCodeword = $t15[$i15];
            $i15 < $l15;
            $i15++, previousRowCodeword = $t15[$i15]
        ) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.detectCodeword = function(
    image,
    minColumn,
    maxColumn,
    leftToRight,
    ...
    startColumn = ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustCodewordStartColumn(
        image,
        minColumn,
        maxColumn,
        leftToRight,
    ...
    var moduleBitCount = ZXing.PDF417.Internal.PDF417ScanningDecoder.getModuleBitCount(
        image,
        minColumn,
        maxColumn,
        leftToRight,
    ...
    var endColumn;
    var codewordBitCount = ZXing.PDF417.PDF417Common.getBitCountSum(
        moduleBitCount
    );
    if (leftToRight) {
    ...
    if (
        !ZXing.PDF417.Internal.PDF417ScanningDecoder.checkCodewordSkew(
            codewordBitCount,
            minCodewordWidth,
            maxCodewordWidth
        )
    ...
    var decodedValue = ZXing.PDF417.Internal.PDF417CodewordDecoder.getDecodedValue(
        moduleBitCount
    );
    ...
    return new ZXing.PDF417.Internal.Codeword(
        startColumn,
        endColumn,
        ZXing.PDF417.Internal.PDF417ScanningDecoder.getCodewordBucketNumber(
            decodedValue
        ),
        codeword
    );
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.getModuleBitCount = function(
    image,
    minColumn,
    maxColumn,
    leftToRight,
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.getNumberOfECCodeWords = function(
    barcodeECLevel
) {
    return 2 << barcodeECLevel;
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.adjustCodewordStartColumn = function(
    image,
    minColumn,
    maxColumn,
    leftToRight,
    ...
    for (var i = 0; i < 2; i++) {
        while (
            ((leftToRight && correctedStartColumn >= minColumn) ||
                (!leftToRight && correctedStartColumn < maxColumn)) &&
            leftToRight == image.get_Item(correctedStartColumn, imageRow)
        ) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.checkCodewordSkew = function(
    codewordSize,
    minCodewordWidth,
    maxCodewordWidth
) {
    return (
        minCodewordWidth - 2 <= codewordSize &&
        codewordSize <= maxCodewordWidth + 2
    );
};
ZXing.PDF417.Internal.PDF417ScanningDecoder.decodeCodewords = function(
    codewords,
    ecLevel,
    erasures
) {
    ...
    var correctedErrorsCount = ZXing.PDF417.Internal.PDF417ScanningDecoder.correctErrors(
        codewords,
        erasures,
        numECCodewords
    );
    ...
    if (
        !ZXing.PDF417.Internal.PDF417ScanningDecoder.verifyCodewordCount(
            codewords,
            numECCodewords
        )
    ) {
    ...
    var decoderResult = ZXing.PDF417.Internal.DecodedBitStreamParser.decode(
        codewords,
        ecLevel.toString()
    );
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.correctErrors = function(
    codewords,
    erasures,
    numECCodewords
) {
    if (
        (erasures != null &&
            erasures.length > Math.floor(numECCodewords / 2) + 3) ||
        numECCodewords < 0 ||
        numECCodewords > 512
    ) {
        return -1;
    ...
    if (
        !(function() {
            ...
            var $res = ZXing.PDF417.Internal.PDF417ScanningDecoder.errorCorrection.decode(
                codewords,
                numECCodewords,
                erasures,
                $1
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.verifyCodewordCount = function(
    codewords,
    numECCodewords
) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.getBitCountForCodeword = function(
    codeword
) {
...
ZXing.PDF417.Internal.PDF417ScanningDecoder.getCodewordBucketNumber = function(
    codeword
) {
    if (typeof codeword == "number") {
        return ZXing.PDF417.Internal.PDF417ScanningDecoder.getCodewordBucketNumber(
            ZXing.PDF417.Internal.PDF417ScanningDecoder.getBitCountForCodeword(
                codeword
            )
        );
    } else {
    ...
    for (var row = 0; row < barcodeMatrix.length; row++) {
        ...
        for (var column = 0; column < barcodeMatrix[row].length; column++) {
            ...
            if (values.length == 0) {
                ...
                formatter += "{0}({1})".format(
                    FormatInteger(values[0], 4),
                    FormatInteger(barcodeValue.getConfidence(values[0]), 2)
                );
            }
        }
        formatter += "\n";
...
ZXing.PDF417.PDF417Reader.decode = function(image, hints, multiple, dr) {
    ...
    var detectorResult =
        dr ||
        ZXing.PDF417.Internal.Detector.detectSingle(image, hints, multiple);
    if (detectorResult) {
        var pointsList = detectorResult.Points;
        for (var $i = 0, $n = pointsList.length; $i < $n; $i++) {
            var points = pointsList[$i];
            var decoderResult = ZXing.PDF417.Internal.PDF417ScanningDecoder.decode(
                detectorResult.Bits,
                points[4],
                points[5],
                points[6],
                points[7],
                ZXing.PDF417.PDF417Reader.getMinCodewordWidth(points),
                ZXing.PDF417.PDF417Reader.getMaxCodewordWidth(points)
            );
            if (!decoderResult) {
                continue;
            }
            var result = new ZXing.Result(
                decoderResult.Text,
            ...
            var pdf417ResultMetadata =
                decoderResult.Other instanceof
                    ZXing.PDF417.PDF417ResultMetadata ||
                decoderResult.Other == null
                    ? decoderResult.Other
                    : (function() {
                          throw new Error("InvalidCastException");
                      })();
...
ZXing.PDF417.PDF417Reader.getMaxCodewordWidth = function(p) {
    return Math.max(
        Math.max(
            ZXing.PDF417.PDF417Reader.getMaxWidth(p[0], p[4]),
            ZXing.PDF417.PDF417Reader.getMaxWidth(p[6], p[2]) *
                Math.floor(
                    ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD /
                        ZXing.PDF417.PDF417Common.MODULES_IN_STOP_PATTERN
                )
        ),
        Math.max(
            ZXing.PDF417.PDF417Reader.getMaxWidth(p[1], p[5]),
            ZXing.PDF417.PDF417Reader.getMaxWidth(p[7], p[3]) *
                Math.floor(
                    ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD /
                        ZXing.PDF417.PDF417Common.MODULES_IN_STOP_PATTERN
                )
...
ZXing.PDF417.PDF417Reader.getMinCodewordWidth = function(p) {
    return Math.min(
        Math.min(
            ZXing.PDF417.PDF417Reader.getMinWidth(p[0], p[4]),
            ZXing.PDF417.PDF417Reader.getMinWidth(p[6], p[2]) *
                Math.floor(
                    ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD /
                        ZXing.PDF417.PDF417Common.MODULES_IN_STOP_PATTERN
                )
        ...
        Math.min(
            ZXing.PDF417.PDF417Reader.getMinWidth(p[1], p[5]),
            ZXing.PDF417.PDF417Reader.getMinWidth(p[7], p[3]) *
                Math.floor(
                    ZXing.PDF417.PDF417Common.MODULES_IN_CODEWORD /
                        ZXing.PDF417.PDF417Common.MODULES_IN_STOP_PATTERN
                )
